...
RGBTRIPLE ApplySepiaFilterToPixel(RGBTRIPLE pixel);
...
void LoopThroughPixels(string filter, RGBTRIPLE(*image)[widthGlobal])
{
    ...
    for (int row = 0; row < heightGlobal; row++)
    {
        ...
        for (int column = 0; column < widthGlobal; column++)
        {
            ...
            {
                tempImage[row][column] = ApplySepiaFilterToPixel(image[row][column]);
            }
...
RGBTRIPLE ApplySepiaFilterToPixel(RGBTRIPLE pixel)
{
    RGBTRIPLEDouble tempPixel = {.rgbtRed = 0, .rgbtGreen = 0, .rgbtBlue = 0};
    RGBTRIPLE returnPixel = {.rgbtRed = 0, .rgbtGreen = 0, .rgbtBlue = 0};

    tempPixel.rgbtRed = round((0.393 * pixel.rgbtRed) + (0.769 * pixel.rgbtGreen) + (0.189 * pixel.rgbtBlue));
    tempPixel.rgbtGreen = round((0.349 * pixel.rgbtRed) + (0.686 * pixel.rgbtGreen) + (0.168 * pixel.rgbtBlue));
    tempPixel.rgbtBlue = round((0.272 * pixel.rgbtRed) + (0.534 * pixel.rgbtGreen) + (0.131 * pixel.rgbtBlue));

    // Cap at 255 for RGB value
    returnPixel.rgbtRed = (tempPixel.rgbtRed > 255) ? 255 : tempPixel.rgbtRed;
    returnPixel.rgbtGreen = (tempPixel.rgbtGreen > 255) ? 255 : tempPixel.rgbtGreen;
    returnPixel.rgbtBlue = (tempPixel.rgbtBlue > 255) ? 255 : tempPixel.rgbtBlue;
