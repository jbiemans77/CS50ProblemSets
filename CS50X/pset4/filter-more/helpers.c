...
void ApplyFilterToAllPixels(string filter, RGBTRIPLE(*image)[widthGlobal]);
...
RGBTRIPLE ApplyFilterUsingValuesFrom3x3GridAroundPixel(string filter, int row, int column, RGBTRIPLE(*image)[widthGlobal]);
...
void grayscale(int height, int width, RGBTRIPLE image[height][width])
{
    ...
    ApplyFilterToAllPixels("grayscale", image);
...
{
    SetWidthAndHeightGlobalVariables(width, height);
    ApplyFilterToAllPixels("horizontalReflection", image);
...
{
    SetWidthAndHeightGlobalVariables(width, height);
    ApplyFilterToAllPixels("blur", image);
...
{
    SetWidthAndHeightGlobalVariables(width, height);
    ApplyFilterToAllPixels("edges", image);
...
void ApplyFilterToAllPixels(string filter, RGBTRIPLE(*image)[widthGlobal])
{
    ...
    for (int row = 0; row < heightGlobal; row++)
    {
        ...
        for (int column = 0; column < widthGlobal; column++)
        {
            ...
            {
                tempImage[row][column] = ApplyFilterUsingValuesFrom3x3GridAroundPixel("blur", row, column, image);
            }
            else if (strcmp(filter, "edges") == 0)
            {
                tempImage[row][column] = ApplyFilterUsingValuesFrom3x3GridAroundPixel("edges", row, column, image);
            }
            else
            {
